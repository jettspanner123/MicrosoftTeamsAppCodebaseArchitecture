# VM uptime request bot

A Teams bot for requesting a VM be kept up outside normal off-hours, with an approve/reject flow.

- Type `/request-vm` to get a form (VM name, from date/time in CET, uptime: 4/8/12/24 hours).
- Submitting sends an Accept/Reject card (with an optional/required note) to a single configured approver.
- Both the requester's and approver's cards update live in place as the request moves from Pending to Accepted/Rejected.
...
1. **Start Azurite** (local Table Storage emulator) in a separate terminal: `npm run azurite`
...
3. Run the bot (see below).

**Important limitation of the M365 Agents Playground**: the Playground simulates a single person chatting with the bot. Proactively notifying a *different* person (the approver) requires the bot to have a real registered identity (Entra app + Bot Framework registration), which the Playground's zero-credential mode doesn't have. So in Playground you can build/verify the form, validation, and card rendering, but the "deliver this to the approver" step needs either the local F5 debug flow (which does register a real Entra app) or a real deployment - both currently blocked on the pending Azure/Entra access request. If proactive delivery fails, the request is still saved and the requester is told delivery failed rather than the bot crashing.
